...
<details>
  ...
  <p>브라우저가 URL에 적힌 값을 파싱해서 HTTP Request Message를 만들고, OS에 전송 요청을 합니다. 이 때, Domain으로 요청을 보낼 수 없기 때문에 우선 DNS Lookup을 수행합니다.</p>
  <p>DNS 룩업 과정은 크롬 클라이언트의 경우 브라우저 → hosts 파일 → DNS Cache의 순서로 도메인에 매칭되는 ip를 찾습니다. 만약 로컬 DNS 캐시에 매핑이 안 되어 있으면, 사용자가 ISP의 DNS 서버에 도메인 이름을 요청합니다. 일반적으로 설명하는 DNS Lookup은 루트 도메인서버에서부터 서브도메인 서버순으로 찾게됩니다. ( 루트  -> TLD -> 서브도메인 )</p>
...
액세스 회선에 의해 통신사용 라우터로 운반되고 인터넷의 핵심부로 전달됩니다. 고속 라우터들 사이로 목적지까지 패킷이 흘러들어가게 됩니다.</p>
  ...
  <p>DNS 서버에 의해 반환된 IP 주소는 이를 클라이언트에 전달하고, 클라이언트는 반환된 IP 주소를 사용헤 WAS에 요청을 보내게 됩니다. </p>
...
<details>
  ...
  <p>TCP(Transmission Control Protocol)는 연결 지향형 프로토콜이고 UDP(User Datagram Protocol)는 데이터를 데이터그램단위로 전송하는 프로토콜입니다.</p>
...
<details>
  ...
  <p>TCP 3way handshake는 가상회선을 수립하는 단계입니다. 클라이언트는 서버에 요청을 전송할 수 있는지, 서버는 클라이언트에게 응답을 전송할 수 있는지 확인하는 과정입니다. SYN, ACK 패킷을 주고받으며, 임의의 난수로 SYN 플래그를 전송하고, ACK 플래그에는 1을 더한값을 전송합니다. 정확한 순서는 ** 클라이언트 SYN(n) -> 서버 측 ACK(n + 1), SYN(m) -> 클라이언트 ACK(m + 1) ** 순으로 일어납니다.</p>
  <p>왜 임의의 난수를 지정하느냐는 꼬리질문이 나올 수 있습니다. 기존 요청과 구분하기 위해서 정도로 알고있고, 그 이상은 생각해본적이 없네요.</p>
  </br>
  <p>TCP 4way handshake는 TCP연결을 해제하는 단계로, 클라이언트는 FIN 패킷을 보내 서버에게 연결해제를 통지하고, 서버가 이를 확인하고 클라이언트에게 이를 받았음을 ACK 패킷으로 전송해주고 최종적으로 연결 종료 상태에 들어갑니다. 이때 서버는 최종적으로  통신이 끝날 때까지 TIME_WAIT 상태가 됩니다. 이후 서버에서 통신이 종료됨을 알리기 위해 클라이언트로 FIN 패킷을 보내고, 클라이언트에서  FIN 패킷을 받고 확인을 알리는 ACK 패킷을 보냅니다. 이때 클라이언트 측에서는 TIME_WAIT상태로 일정시간 대기하는데, 혹시나 패킷이 나중에 도착할 수 있기 때문입니다.</p>
</details>

<details>
  <summary>HTTP와 HTTPS의 차이점에 대해서 설명해보세요.</summary>
  </br>
  <p>HTTP 프로토콜은 따로 암호화 과정을 거치지 않기 때문에 중간에 패킷을 가로챌 수 있고, 수정할 수 있습니다. 따라서 보안이 취약해짐을 알 수 있습니다. 이를 보완하기 위해 나온 것이 HTTPS입니다. HTTPS는 바로 TCP로 통신하지 않고, SSL로 통신하고 SSL이 TCP와 통신합니다. TCP의 3-way handshake이 선행되고 이후 SSL Handshake가 수행됩니다.</p>
</details>
...
<details>
  ...
  <p>HTTPS의 암호화 방식은 대칭키와 비대칭키 방식을 모두 사용합니다. 브라우저는 접속하려는 웹사이트의 인증서를 확인합니다. 인증기관에서 얻은 공개키로 인증서를 검증하면 사이트의 정보와 서버의 공개키를 얻게 됩니다. 이 공개키로 대칭키를 암호화해 전송합니다. 서버는 대칭키를 사이트 개인키로 복호화하고, 이후 통신에 해당 대칭키를 사용해 통신합니다.</p>
...
<details>
  ...
  <p>동기/비동기는 특정 작업 수행시 해당 **작업의 완료 여부**에 따라 구분합니다. 메서드 리턴과 결과를 전달받는 시간이 일치하면 동기입니다. 또, 동기에서는 리턴 받더라도 미완료 상태인 경우 작업 완료 여부를 계속 확인합니다.</p>
  <p>블로킹/논블로킹은 동기,비동기와는 다른 관점입니다. 작업을 수행할 때 제어권이 있는지에 따라 구분하며, IO/멀티스레드 등을 상대하는 방법입니다.</p>
...
<details>
  ...
  <p>정규화는 1차 정규화부터 5차 정규화까지 총 5단계로 구성되어 있고, 정규화를 통해 데이터 일관성을 유지하고 이상 현상을 방지할 수 있습니다.</p>
  <p> 하지만 과도한 정규화는 쿼리 처리 속도를 떨어뜨리고, 테이블 간의 조인이 복잡해질 수 있으므로 적절한 수준의 정규화가 필요합니다.</p>
</details>

<details>
  <summary>JOIN에 대해서 설명해주세요.</summary>
  </br>
  <p>데이터베이스에서 여러 테이블을 조합해 데이터를 얻어내고자 할때 Join을 사용하는 것으로 알고 있습니다.</p>
  <p>
  <img src = "https://user-images.githubusercontent.com/92436863/233326557-2af9207e-6cbc-45ff-a94c-b0c9c8169d7e.png">
  내부 조인, 외부 조인 등이 있으며 셀프 조인으로 자기 자신과 조인해 데이터를 추출할 수도 있습니다.
  </p>
...
<details>
  ...
  <p>MongoDB는 NoSQL 데이터베이스의 한 종류로, 문서 기반(Document-Oriented) 데이터 모델을 사용합니다.
데이터를 JSON과 유사한 BSON 형식으로 저장하며, 스키마가 유연하다는 특징이 있습니다.</p>
  <p>왜 MongoDB를 선택해야 하는가? -> 빠르게 변화하는 요구사항이나 대량의 데이터를 유연하게 처리해야 할 때 적합합니다.</p>
  <p>MongoDB를 언제 쓰지 않아야 하는가? -> 데이터의 강한 정합성(Consistency)이나 복잡한 관계 모델링이 필요한 경우, 관계형 데이터베이스가 더 적합할 수 있습니다.</p>
  <p>Aggregation Framework을 통해 복잡한 데이터 처리가 가능합니다.</p>
</details>

<details>
  <summary>기존 쿼리를 개선할때 수행할 작업</summary>
  </br>
  <p>쿼리 개선의 목적이 무엇인지 명확히 합니다. 예를 들어, 성능 최적화, 리소스 절약, 혹은 비즈니스 요구사항 충족 등이 될 수 있습니다.</p>
  <p>
    <ul>
      <li>불필요한 데이터 제거: "필요 없는 컬럼이나 조건을 제거해 쿼리를 간소화합니다."</li>
      <li>인덱스 최적화: "인덱스가 적절히 사용되고 있는지 확인하거나 필요한 인덱스를 추가합니다."</li>
      <li>쿼리 구조 개선: "서브쿼리를 조인으로 대체하거나, 비효율적인 JOIN을 재구성합니다."</li>
      <li>데이터베이스 설정 점검: "테이블 파티셔닝, 캐싱, 혹은 데이터베이스 설정을 확인하여 병목을 완화합니다."</li>
    </ul>
  </p>
  <p>기존 쿼리의 성능과 목적을 분석한 후, 실행 계획(EXPLAIN)을 통해 병목 지점을 파악합니다. 그 후 쿼리 구조를 간소화하거나 인덱스를 최적화하며, 최종적으로 성능을 검증합니다.</p>
</details>
...
<details>
  ...
  <p> 
  SQL 인젝션이란, 악의적인 사용자가 응용 프로그램 보안 상의 허점을 의도적으로 이용해, 악의적인 SQL문을 주입하고 실행되게 하여 데이터베이스가 비정상적인 동작을 하도록 조작하는 행위입니다. 주로 사용자가 입력한 데이터를 제대로 필터링, 이스케이핑하지 못했을 경우에 발생합니다. 
  </p>
</details>
...
<details>
  ...
  <p> XSS(Cross-Site Scripting, 크로스사이트 스크립팅)은 공격자가 입력한 악성스크립트가 사용자 측에서 응답하는 취약점으로, 사용자 입력값에 대한 검증이 미흡하거나 출력 시 필터링 되지 않을 경우 발생합니다. 쿠키 값 또는 세션 등 사용자의 정보를 탈취하거나 피싱 사이트로의 접근 유도 등 사용자에게 직접적인 피해를 줄 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
  웹 어플리케이션 취약점 중 하나로 인터넷 사용자가 자신의 의지와는 다르게 공격자가 의도한 수정, 삭제, 등록 등의 행위를 사용자가 사용하는 웹 사이트에 요청하게 만드는 공격입니다.
  </p>
</details>
...
<details>
  ...
  <p>
  JVM은 클래스 파일을 로드하고 링크로 배치하는 클래스 로더, 메모리 영역인 Runtime Data Area, Execution Engine, Garbage Collector로 구성되어 있습니다.
  </p>
</details>
...
<details>
  ...
  <p> 
  JVM의 Runtime Data Area는 클래스가 로드될 때 할당되며 클래스 메소드, static 변수, constant pool 정보 등이 저장되는 Method Area, 런타임 시 할당되며 동적할당된 객체와 배열이 저장되는 영역인 Heap Area, 컴파일 시 할당되며 지역변수, 파라미터, 리턴값 등 임시값이 할당되는 Stack Area, C와 같이 자바 외 언어로 작성된 네이티브 코드를 위한 메모리 영역인 Native Method Area, 스레드가 생성될 때 할당되며 현재 스레드가 실행되는 부분의 주소와 명령 등을 저장하는 PC register로 구성됩니다.
  </p>
</details>
...
<details>
  ...
  <p>
  데이터를 쉽고 효과적으로 처리하기 위해 표준화된 방법을 제공하는 클래스의 집합입니다. List, Set, Map 등의 클래스가 존재합니다.
  </p>
</details>
...
<details>
  ...
  <p>
  객체 생성 시점에 타입을 결정하여, 유연한 설계를 가능하게 하는 기능입니다. 최상위 클래스인 Object가 아닌 제너릭을 사용하는 이유는, 컴파일 시 타입 안정성을 보장하기 위함입니다.
  </p>
</details>
...
<details>
  ...
  <p>
  사전적 의미로는 주석이라는 뜻을 가지며, 소스코드에 추가해 사용할 수 있는 일종의 메타데이터이며 보통 @ 기호를 앞에 붙여 사용합니다. 자바에서 기본적으로 제공하는 표준 어노테이션과 어노테이션 정의를 위해 사용되는 메타 어노테이션, 사용자가 정의해 사용할 수 있는 사용자 어노테이션이 존재합니다.
  </p>
</details>
...
<details>
  ...
  <p>
  인터페이스는 클래스에서 메소드 구현을 강제하기 위해 사용하며, 다중 상속(엄밀히 말하면 implement)가 가능합니다. java 8부터 default 메소드를 통해 기본 메서드 구현이 가능합니다. 추상 클래스는 다중 상속이 불가하며, 필드 값과 일반 메서드를 가질 수 있습니다. 
  </p>
</details>
...
<details>
  ...
  <p>
  클래스란 객체를 만들어내기 위한 템플릿 혹은 청사진이고, 객체가 소유할 일련의 속성과 행동을 정의합니다. 객체는 이러한 클래스를 이용해 만들어진 클래스의 인스턴스로, 속성을 가지고 메소드를 사용할 수 있습니다.
  </p>
</details>
...
- 개방-폐쇄 원칙 (Open/Closed Principle)
	...
	-   새로운 기능을 추가할 때는 **기존의 코드를 변경하지 않고, 확장**을 통해 추가해야 한다.
...
- 인터페이스 분리 원칙 (Interface Segregation Principle)
	-   클라이언트는 자신이 사용하지 않는 인터페이스를 구현해서는 안된다.
	-   하나의 일반적인 인터페이스보다 여러개의 구체적인 인터페이스가 낫다.

-  의존 역전 원칙 (Dependency Inversion Principle)
	-   고차원 모듈은 저차원 모듈에 의존해서는 안 되며, 둘 다 **추상화에 의존**해야 한다.
	-   **인터페이스나 추상 클래스와 같은 추상화**를 통해 의존성을 역전시켜야 한다.
  </p>
</details>
...
<details>
  ...
  <p>
    직렬화란 객체들의 데이터를 연속적인 데이터(스트림)로 변형하여 전송 가능한 형태로 만드는 것이고, 역직렬화는 직렬화된 데이터를 다시 객체의 형태로 만드는 것입니다.
  </p>
  <p>
    Heap 영역의 객체는 스택에서 객체를 주소값을 참조해 사용하게 됩니다. 이 주소값을 그대로 다른 곳에 보내게 되면, 프로그램이 종료되거나 객체가 사라졌을때 무의미한 주소값만 남게 될 것입니다. 따라서 직렬화를 통해 파일 저장이나 네트워크 전송시 유의미한 데이터를 넘겨줄 수 있도록 해야합니다. 
  </p>
</details>
...
<details>
  ...
  <p>
  첫번째 처리 방법으로는 문제가 된 메서드, 변수에 각각 synchronized라는 키워드를 넣는 암시적 Lock 방식이 있습니다. 이 방식은 간단하지만, 해당 영역에서 처리되는 로직이 많으면 많을수록 다른 스레드 작업이 지연되어 자원 낭비가 발생하게 됩니다.
  </p>
  <p>
  두번째 처리 방법으로는 Thread-safe한 객체를 사용하는 것입니다. concurrent 패키지에서 이러한 객체를 제공하고, 락을 최소화해 동시성 이슈가 발생하지 않게 합니다.
    <p>
    불변 객체를 사용할 수도 있습니다. 불변 객체는 내부 상태가 변하지 않으므로 Thread-safe하기 때문입니다. 가장 간단한 방법은 세터(setter)를 만들지 않는 것이고, 내부 상태가 변하지 않도록 모든 변수를 final로 선언할 수도 있습니다.
    </p>
  </p>
</details>
...
<details>
  <summary>JDK 8과 JDK 17의 주요 차이점에 대해 설명해주세요.</summary>
  </br>
  <p>JDK 8은 여전히 널리 사용되지만, JDK 17은 최신 프로젝트에 적합하며, 특히 성능과 보안이 중요한 경우 사용이 권장됩니다. JDK 17의 새로운 기능들은 코드 간결성과 유지보수성 측면에서 큰 장점을 제공합니다. 예를 들어, Records를 활용하면 데이터 클래스를 쉽게 정의할 수 있고, Sealed Classes로 클래스 계층 구조를 안전하게 설계할 수 있습니다. 또한, JDK 17은 G1 GC를 기본으로 하며 ZGC와 같은 저지연 GC를 지원합니다.</p>
</details>
...
Spring은 Java 애플리케이션 개발을 간편하게 해주는 강력한 프레임워크로, 다양한 기능(예: 의존성 주입, 트랜잭션 관리 등)을 제공합니다. 하지만 설정이 복잡하고 XML 설정 파일이나 여러 Java 설정 클래스가 필요할 수 있습니다.
  <br></br>
Spring Boot는 Spring을 기반으로 하지만 설정을 간소화하는 데 중점을 둔 도구입니다. 
  <li> 내장 웹 서버(WAS): Spring Boot는 Tomcat, Jetty 등을 내장하고 있어 별도의 WAS 설치 없이 애플리케이션을 실행할 수 있습니다. </li>
  <li> 자동 설정(Auto Configuration): Spring Boot는 프로젝트에서 사용하는 라이브러리를 기반으로 적절한 기본 설정을 자동으로 제공해 개발자의 부담을 줄여줍니다.</li>
  <li> 의존성 관리: Spring Boot Starter라는 의존성 집합을 제공해 관련 라이브러리를 쉽게 관리할 수 있습니다. </li>
  </p>
</details>
...
<details>
  ...
  <p>
  스프링 컨테이너에 의해 관리되는 재사용 가능한 소프트웨어 컴포넌트를 Bean이라 하며, 스프링 컨테이너에 등록된 객체를 스프링 빈이라고 합니다. 스프링 간 객체가 의존관계를 관리하도록 할 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
Spring 내 생성 과정 : 객체 생성 -> **의존성 객체 주입** -> 의존성 객체 메소드 호출
  </p>
  <p>
개발자가 Bean 등록하는 방법
...
<details>
  ...
  <p>
스코프란, 스프링 어플리케이션이 구동되는 동안 Bean이 존재할 수 있는 범위를 뜻합니다.
...
기본적으로 객체를 생성하고 객체간의 의존성을 이어주는 역할을 수행합니다.
  </p>
</details>
...
<details>
  ...
  <p>
Setter Injection, Constructor Injection, 필드 주입 세 가지 방식이 있습니다. 이 세 가지 방식은 클래스 내 선언되는 위치에 따라 구분되고, 보통 생성자 주입이 권장됩니다.
<br></br>
생성자 주입으로 순환 참조를 방지하고, 생성자 주입을 통해 변경의 가능성을 배제하고 불변성을 보장할 수 있습니다. 또한 생성자 주입을 사용하게 되면 테스트 코드를 좀 더 편리하게 작성할 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
  @Autowired 는 Spring에게 의존성을 주입하는 지시자 역할을 합니다.
  </p>
</details>
...
<details>
  ...
  <p>
@Component`는 Class Level에서, `@Bean`은 Method Level에서 적용됩니다. `@Bean`은 사용자가 컨트롤 하지 못하는 Class나, 좀 더 유연하게 객체를 생성해서 넘기고 싶을 때 (이를 테면 외부 라이브러리) 사용합니다. `@Component`는 Class 자체를 빈으로 등록하고 싶을 때 사용한다.
  </p>
</details>
...
<details>
  ...
  <p>
POJO란 Plain Old Java Object의 약자로 다른 클래스나 인터페이스를 상속/implements 받아 메서드가 추가된 클래스가 아닌 getter, setter 같이 기본적인 기능만 가진 자바 객체를 말합니다.
이런 Pojo는 스프링에서 주로 데이터 전송 객체(DTO)로 사용되며, 데이터베이스에서 검색한 결과를 자바 객체로 변환하는 데 사용됩니다.
  </p>
</details>
...
<details>
  <summary>ORM과 순수 쿼리 방식의 차이점을 설명해주세요.</summary>
  </br>
  <p> 
  ORM은 객체지향적으로 데이터를 처리하며, DB 테이블과 클래스 간 매핑이 필요합니다. 개발자가 SQL을 직접 작성하지 않아도, ORM 프레임워크가 객체의 상태에 따라 SQL을 생성 및 실행합니다. 이 예로 Hibernate와 JPA 등이 있습니다. DB 작업이 객체와 메서드 기반으로 이루어지기 때문에 직관적이고 빠른 개발이 가능합니다. 또한, 스키마 변경에 대해 ORM 매핑만 수정하면 되며 코드 재사용성이 높아 유지보수가 용이합니다. 대신 N + 1 문제 등 성능 이슈가 발생할 수 있습니다.
  </p>
  <p>
  순수 쿼리는 쿼리의 모든 기능(서브쿼리, join 등)을 활용할 수 있으며, 제약이 적습니다. 대신 쿼리를 직접 작성해야 하므로 개발 속도는 상대적으로 느릴 수 있으며, 스키마 변경이 일어날 경우 여러 곳을 수정해야 할 수 있습니다. 하지만 복잡한 비즈니스 로직을 구현하고자 할 때 성능 최적화가 용이합니다. (쿼리 최적화)
  </p>
  <p>
  대다수의 프로젝트에서는 ORM과 순수 쿼리를 **혼합 사용**하여, 간단한 작업은 ORM으로 처리하고, 성능이 중요한 작업은 순수 쿼리를 활용합니다.
  </p>
</details>
...
<details>
  ...
  <p>
SQL 쿼리를 직접 생성하지 않고, JPA에 저장할 객체를 전달하고 JPA 메소드를 활용해 데이터 베이스를 다룰 수 있습니다. 또, SQL 의존적인 개발은 엔티티 컬럼이 변경되는 상황에서 연관된 모든 SQL문을 수정해야하고 결과를 매핑하기 위한 JDBC API도 변경해야 하지만 JPA는 JPA에서는 엔티티 클래스 정보만 변경하면 됩니다. DB 벤더와 독립적으로 작동하기 때문에 로컬 개발 환경과 상용 환경 등의 데이터베이스가 상이하더라도 변경 작업이 간편합니다. 또한, 데이터 베이스에 맞춰 데이터를 저장하기 위해 객체를 매핑하는 과정에서 드는 비용을 없애 좀 더 객체지향적인 개발이 가능합니다. 같은 트랜잭션 내에서는 캐싱 기능을 제공하는 등 성능 이점도 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>만약 그런 객체를 가져와야 하는 경우 Fetch Join이라고 하는 JPQL의 join fetch를 사용합니다. 쿼리 한 번으로 해결할 수 있고, 또 다른 방법으로는 EntityGraph를 사용하는 방법이 있습니다. QueryDSL과 같이, 쿼리 빌더를 사용해 해결할 수도 있습니다.</p>
</details>
...
<details>
  <summary>데코레이터 함수에 대해 설명해주세요.</summary>
  </br>
  <p>파이썬에서 데코레이터는 함수를 수정하거나 확장할 때 사용되는 디자인 패턴입니다. 이는 다른 함수를 감싸는 래퍼 함수로, 기존 함수의 동작을 유지하면서 추가적인 기능을 더할 수 있습니다. 데코레이터는 주로 `@`(데코레이터) 문법을 사용해 정의하며, 코드의 재사용성과 가독성을 높이는 데 유용합니다.</p>
  <p>Python의 데코레이터는 Java의 AOP와 매우 유사합니다. 둘 다 실행 흐름 전후에 특정 로직을 추가하는 데 사용됩니다. 다만, Java AOP는 주로 **프록시 객체와 어노테이션**을 활용해 동작하며, Python 데코레이터는 **함수 자체를 래핑**해 동적으로 동작을 제어합니다. 두 개념 모두 코드의 가독성을 높이고 부가 로직을 분리하는 데 효과적이라는 공통점이 있습니다.</p>
</details>
...
<details>
  ...
  <p>
로드밸런서는 서버의 부하를 분산시켜주는 장치나 소프트웨어로, 한 서버에 부하가 집중되면 로드밸런서에서 부하가 적은 서버로 요청을 전달해 서버의 안정성을 유지합니다. 대규모 서비스에 이를 적용해 서버 수를 늘리고 안정적으로 서비스를 제공할 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
도커란 일종의 컨테이너이며, 컨테이너의 생성 및 관리 프로세스를 단순화하는 도구로 빌드 및 관리를 돕습니다. 이 컨테이너는 일종의 표준화된 소프트웨어 유닛으로, 컨테이너 내에 다양한 소프트웨어 유닛이나 코드를 담을수 있고 넣은 뒤에는 자체적으로 보관 및 격리됩니다. 이런 컨테이너 기술을 사용하면, 어디에서 누가 실행하더라도 동일한 환경에서 동일한 어플리케이션을 실행할 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
도커와 가상 머신(VM)은 애플리케이션 배포에 사용되는 두 가지 기술입니다. 도커는 개발자가 컨테이너라는 표준화된 단위로 소프트웨어를 패키징하는 데 사용하는 오픈 소스 플랫폼입니다. 컨테이너에는 라이브러리, 시스템 도구, 런타임을 비롯한 애플리케이션 코드와 해당 환경이 모두 갖춰져 있습니다. 도커를 사용하면 어떤 시스템에서든 애플리케이션을 배포하고 규모를 조정하며 코드가 일관되게 실행되도록 할 수 있습니다. 반면 가상 머신은 물리적 머신의 디지털 사본으로, 동일한 호스트 운영 체제에서 고유한 개별 운영 체제가 실행되는 여러 가상 머신을 보유할 수 있습니다. 가상 머신에서 도커 컨테이너를 실행할 수도 있습니다.
<br></br>
VM은 원래 단일 물리적 시스템에서 여러 운영 체제를 실행할 수 있도록 설계되었고, 사용자가 기본 하드웨어로부터 격리된 가상 환경을 만들 수 있도록 합니다. VM은 하드웨어 세부 정보를 추상화하여 다양한 하드웨어 아키텍처에서 애플리케이션을 보다 쉽게 실행하고 하드웨어 리소스를 더 효율적으로 사용할 수 있도록 합니다. 반면 도커는 격리되고 재현 가능한 환경에서 애플리케이션을 패키징하고 실행할 수 있는 가볍고 이동성이 뛰어난 방법을 제공하도록 설계되었습니다. 도커는 운영 체제 세부 정보를 추상화하여 개발, 테스트 및 프로덕션과 같은 다양한 환경에 애플리케이션 배포 시의 문제를 해결합니다. 수백 개의 애플리케이션을 실행하거나 애플리케이션을 수백 개의 마이크로서비스로 분해하는 조직의 경우 특히 그렇습니다. 도커는 컨테이너화를 통해 소프트웨어 환경 업데이트를 관리하고 어디서나 환경 일관성을 유지할 수 있습니다.
  </p>
</details>
...
<details>
  ...
  <p>
CI(Continuous Integration), (Continuous Delivery/Deployment)는 코드 변경 사항을 공유 소스 코드 **리포지토리**를 **자주 통합**하는 지속적 통합과 CI를 통해 최종적인 빌드 산출물을 사용자에게, 즉 프로덕션 환경에 자동 배포하는 것을 의미하는 것으로 알고 있습니다. 이와 같이 코드를 빌드, 테스트, 배포하는 과정을 거쳐 소프트웨어 개발을 추진하는 프로세스를 CI/CD 파이프라인이라 합니다.
  </p>
</details>
